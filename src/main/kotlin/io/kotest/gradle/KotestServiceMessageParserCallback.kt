...
class KotestServiceMessageParserCallback(private val root: DefaultTestSuiteDescriptor,
   ...
   private fun createDescriptor(msg: ServiceMessage): KotestTestDescriptor {
      ...
      val desc = when (msg.attributes["test_type"]?.toLowerCase()) {
   ...
   private fun notifyAfterTest(msg: ServiceMessage) {
      ...
      val errors = when (resultType) {
         TestResult.ResultType.FAILURE -> RuntimeException(msg.attributes["message"])
         else -> null
      }
   ...
   private fun notifyAfterTestSuite(msg: ServiceMessage) {
      ...
      val resultType = when (msg.attributes["result_status"]?.toLowerCase()) {
